...
static std::uintptr_t findPattern(const char* moduleName, std::string_view pattern, bool reportNotFound = true) noexcept
{
    ...
    if(reportNotFound)
        MessageBoxA(NULL, ("Failed to find pattern #" + std::to_string(id) + '!').c_str(), "Nexus", MB_OK | MB_ICONWARNING);
    return 0;
}
...
Memory::Memory() noexcept
{
    ...
    staticScopeLensClear = findPattern(CLIENT_DLL, "\x0F\x82????\xFF\x50\x3C") + 9;
    staticScopeLensBlurry = findPattern(CLIENT_DLL, "\xFF\xB7????\x8B\x01\xFF\x90????\x8B\x4C\x24\x24") - 6;
}
