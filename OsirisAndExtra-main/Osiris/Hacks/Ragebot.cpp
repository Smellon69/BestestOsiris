...
void runRagebot(UserCmd* cmd, Entity* entity, matrix3x4* matrix, Ragebot::Enemies target, std::array<bool, Hitboxes::Max> hitbox, Entity* activeWeapon, int weaponIndex, Vector localPlayerEyePosition, Vector aimPunch, int multiPoint, int minDamage, float& damageDiff, Vector& bestAngle, Vector& bestTarget) noexcept
{
    ...
    {
        if (cfg[weaponIndex].relativeHitchanceOn && !AimbotFunction::relativeHitChance(localPlayer.get(), entity, set, matrix, activeWeapon, bestAngle, cmd, cfg[weaponIndex].relativeHitchance)) {
            bestTarget = Vector{ };
            bestAngle = Vector{ };
            damageDiff = FLT_MAX;
        }
        else if (!cfg[weaponIndex].relativeHitchanceOn && !AimbotFunction::hitChance(localPlayer.get(), entity, set, matrix, activeWeapon, bestAngle, cmd, cfg[weaponIndex].hitChance))
        {
            bestTarget = Vector{ };
            bestAngle = Vector{ };
            damageDiff = FLT_MAX;
