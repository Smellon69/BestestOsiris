        ...
        if (show && config->misc.jumpStats.enabled)
        {
            ...
            if (jumps > 2)
                memory->clientMode->getHudChat()->printf(0,
                    " \x0C\u2022Nexus\u2022\x01 %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%d\x01 Bhops | \x05%.0f\x01 Sync]",
                    color, jump.c_str(),
                ...
                memory->clientMode->getHudChat()->printf(0,
                    " \x0C\u2022Nexus\u2022\x01 %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%.0f\x01 Sync]",
                    color, jump.c_str(),
    ...
    if (config->misc.faketag != 0)
    {
        ...
        if (time != lastTime)
        {
            ...
            if (tag == 4)
            {
                ...
                {
                case 1: { memory->setClanTag(" | ", " | "); break; }
                case 2: { memory->setClanTag(" N/ ", " N/ "); break; }
                case 3: { memory->setClanTag(" Ne\\ ", " Ne\\ "); break; }
                case 4: { memory->setClanTag(" Nex/ ", " Nex/ "); break; }
                case 5: { memory->setClanTag(" Nexu\\ ", " Nexu\\ "); break; }
                case 6: { memory->setClanTag(" Nexus/ ", " Nexus/ "); break; }
                case 7: { memory->setClanTag(" Nexus\\ ", " Nexus\\ "); break; }
                case 8: { memory->setClanTag(" Nexus./ ", " Nexus./ "); break; }
                case 9: { memory->setClanTag(" Nexus.n\\ ", " Nexus.n\\ "); break; }
                case 10: { memory->setClanTag(" Nexus.ni/ ", " Nexus.ni/ "); break; }
                case 11: { memory->setClanTag(" Nexus.nig ", " Nexus.nig "); break; }
                case 12: { memory->setClanTag(" Nexus.nigg ", " Nexus.nigg "); break; }
                case 13: { memory->setClanTag(" Nexus.nigge ", " Nexus.nigge "); break; }
                case 14: { memory->setClanTag(" Nexus.nigger ", " Nexus.nigger "); break; }
                case 15: { memory->setClanTag(" Nexus.nigger ", " Nexus.nigger "); break; }
                case 16: { memory->setClanTag(" Nexus.nigge\\ ", " Nexus.nigge\\ "); break; }
                case 17: { memory->setClanTag(" Nexus.nigg/ ", " Nexus.nigg/ "); break; }
                case 18: { memory->setClanTag(" Nexus.nig\\ ", " Nexus.nig\\ "); break; }
                case 19: { memory->setClanTag(" Nexus.ni/ ", " Nexus.ni/ "); break; }
                case 20: { memory->setClanTag(" Nexus.n\\ ", " Nexus.n\\ "); break; }
                case 21: { memory->setClanTag(" Nexus./ ", " Nexus./ "); break; }
                case 22: { memory->setClanTag(" Nexus\\ ", " Nexus\\ "); break; }
                case 23: { memory->setClanTag(" Nexu/ ", " Nexu/ "); break; }
                case 24: { memory->setClanTag(" Nex\\ ", " Nex\\ "); break; }
                case 25: { memory->setClanTag(" Ne/ ", " Ne/ "); break; }
                case 26: { memory->setClanTag(" N\\ ", " N\\ "); break; }
                }
            }
            if (tag == 5)
            {
                ...
                {
                case 1: { memory->setClanTag("〄", "〄"); break; }
                case 2: { memory->setClanTag("R>|〄", "R>|〄"); break; }
                case 3: { memory->setClanTag("RA>|〄", "RA>|〄"); break; }
                case 4: { memory->setClanTag("RAW>|〄", "RAW>|〄"); break; }
                case 5: { memory->setClanTag("RAWЭ>|〄", "RAWЭ>|〄"); break; }
                case 6: { memory->setClanTag("RAWЭT>|〄", "RAWЭT>|〄"); break; }
                case 7: { memory->setClanTag("RAWΣTR>|〄", "RAWΣTR>|〄"); break; }
                case 8: { memory->setClanTag("Я4WΣTRI>|〄", "Я4WΣTRI>|〄"); break; }
                case 9: { memory->setClanTag("RAWETRIP>|〄", "RAWETRIP>|〄"); break; }
                case 10: { memory->setClanTag("RAWETRIP<|〄", "RAWETRIP<|〄"); break; }
                case 11: { memory->setClanTag("Я4WΣTRI<|〄", "Я4WΣTRI<|〄"); break; }
                case 12: { memory->setClanTag("RAWΣTR<|〄", "RAWΣTR<|〄"); break; }
                case 13: { memory->setClanTag("RAWЭT<|〄", "RAWЭT<|〄"); break; }
                case 14: { memory->setClanTag("RAWЭ<|〄", "RAWЭ<|〄"); break; }
                case 15: { memory->setClanTag("RAW<|〄", "RAW<|〄"); break; }
                case 16: { memory->setClanTag("RA<|〄", "RA<|〄"); break; }
                case 17: { memory->setClanTag("R<|〄", "R<|〄"); break; }
...
{
    const auto alpha = Helpers::calculateColor(color, color, color, 185);
    // left
    drawList->AddRectFilled(ImVec2{ pos.x - config->misc.scopeOffset, pos.y }, ImVec2{ pos.x - config->misc.scopeLength - config->misc.scopeOffset, pos.y + 1 }, color);

    // right
    drawList->AddRectFilled(ImVec2{ pos.x + config->misc.scopeLength + config->misc.scopeOffset, pos.y }, ImVec2{ pos.x + config->misc.scopeOffset, pos.y + 1 }, color);

    // top (left with swapped x/y offsets)
    drawList->AddRectFilled(ImVec2{ pos.x, pos.y - config->misc.scopeOffset }, ImVec2{ pos.x + 1, pos.y - config->misc.scopeLength - config->misc.scopeOffset }, color);

    // bottom (right with swapped x/y offsets)
    drawList->AddRectFilled(ImVec2{ pos.x, pos.y + config->misc.scopeLength + config->misc.scopeOffset }, ImVec2{ pos.x + 1, pos.y + config->misc.scopeOffset }, color);
}

void Misc::advancedScope(ImDrawList* drawList) noexcept
{
    if (!config->misc.advancedScope.enabled)
        return;
    ...
    drawCrosshair(drawList, ImGui::GetIO().DisplaySize / 2, Helpers::calculateColor(config->misc.advancedScope));
}
...
{
    if (!config->misc.aaArrows.enabled)
        return;
    int x, y;
    interfaces->engine->getScreenSize(x, y);
    constexpr auto triangleSize = 21.f;
    const auto& displaySize = ImGui::GetIO().DisplaySize;
    const auto pos = ImGui::GetIO().DisplaySize / 2 + ImVec2{ static_cast<float> (x), static_cast<float> (y) } *200;
    const auto trianglePos = pos + ImVec2{ static_cast<float>(x), static_cast<float>(y) };
    const auto color = Helpers::calculateColor(config->misc.aaArrows);
    const ImVec2 trianglePoints[]{
    trianglePos + ImVec2{  0.5f, -0.5f } *20.0f,
    trianglePos + ImVec2{  1.f,  1.f } *20.0f,
    trianglePos + ImVec2{ -0.5f,  0.5f } *20.0f,
    };
...
void Misc::StatusBar()noexcept
{
    ...
    ImGui::SetNextWindowSize({ 200.f, 0.f }, ImGuiCond_Once);
    ImGui::SetNextWindowSizeConstraints({ 200.f, 0.f }, { 200.f, FLT_MAX });

    ImGuiWindowFlags windowFlags = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoTitleBar;
    if (!gui->isOpen())
        windowFlags |= ImGuiWindowFlags_NoInputs;

    ImGui::SetNextWindowBgAlpha(0.0f);
    //ImGui::SetWindowPos({ screenWidth - ImGui::GetWindowSize().x - 9, 9 });
    ImGui::GetBackgroundDrawList();
    ImColor color = Helpers::calculateColor(config->misc.indicators);
    ImGui::PushStyleVar(ImGuiStyleVar_WindowTitleAlign, { 0.5f, 0.5f });
    ImGui::PushStyleColor(ImGuiCol_Text, Helpers::calculateColor(config->misc.indicators));
    //Style->Colors[ImGuiCol_Text] = color;
    ImGui::Begin("Indicators", nullptr, windowFlags);
    ImGui::PushID("Cock");
    int screenSizeX, screenSizeY;
    interfaces->engine->getScreenSize(screenSizeX, screenSizeY);
    ImGui::SetWindowPos({ 1, static_cast<float>(screenSizeY / 2) });
    ImGui::PushFont(gui->getTahoma28Font());
    if (config->tickbase.doubletap.isActive() && !localPlayer->isInReload())
        ImGui::Text("DT");
    ...
    if (config->misc.autoPeekKey.isActive() && config->misc.autoPeek.enabled)
        ImGui::Text("PEEK");
    if ((config->misc.edgeBugKey.isActive() && config->misc.edgeBug) || (config->misc.edgeJumpKey.isActive() && config->misc.edgeJump))
        ImGui::Text("EDGE");
    if (config->misc.jumpBug && config->misc.jumpBugKey.isActive())
        ImGui::Text("JB");
    if (config->misc.fakeduck && config->misc.fakeduckKey.isActive())
    ...
    if (config->manualForward.isActive())
        ImGui::Text("AA : FORWARD");
    if (config->manualBackward.isActive())
        ImGui::Text("AA : BACKWARD");
    if (config->triggerbotKey.isActive() && config->triggerbotKey.isSet())
        ImGui::Text("TRIGGER");
...
void Misc::voteRevealer(GameEvent& event) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Nexus\u2022 %c%s\x01 voted %c%s\x01", isLocal ? '\x01' : color, isLocal ? "You" : entity->getPlayerName().c_str(), color, votedYes ? "Yes" : "No");
}
