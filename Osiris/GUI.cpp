...
void GUI::renderFakelagWindow() noexcept
{
    ...
    if (current_category == 6){
        ImGui::Text("You can not configure fakelag during fakeduck");
        ImGui::Text("since it have to choke as much as possible");
        return;
    }
    ImGui::Checkbox("Enabled", &config->fakelag[current_category].enabled);
    ImGui::Combo("Mode", &config->fakelag[current_category].mode, "Static\0Adaptative\0Random\0m1tZw tank\0");
    ImGui::PushItemWidth(220.0f);
...
void GUI::renderRageAntiAimWindow() noexcept
{
    ...
    ImGui::Combo("Yaw base", reinterpret_cast<int*>(&config->rageAntiAim[current_category].yawBase), "Off\0Forward\0Backward\0Right\0Left\0Spin\0");
    ...
    { 
        ImGui::SliderInt("Jitter yaw limit", &config->rageAntiAim[current_category].jitterRange, 0, 90, "%d");
        ImGui::SliderInt("Jitter yaw min", &config->rageAntiAim[current_category].jitterMin, 0, config->rageAntiAim[current_category].jitterRange, "%d");
    }

    if (config->rageAntiAim[current_category].yawBase == Yaw::spin)
    {
        ImGui::PushItemWidth(220.0f);
        ImGui::SliderInt("Spin tick", &config->rageAntiAim[current_category].spinBase, 0, 1280, "%d");
        ImGui::PopItemWidth();
    }
...
void GUI::renderFakeAngleWindow() noexcept
{
    ...
    ImGui::SliderInt("Left min", &config->fakeAngle[current_category].leftMin, 0, config->fakeAngle[current_category].leftLimit - 1, "%d");
    ImGui::PopItemWidth();

    ImGui::PushItemWidth(220.0f);
    ImGui::SliderInt("Right limit", &config->fakeAngle[current_category].rightLimit, 0, 60, "%d");
    ImGui::SliderInt("Right min", &config->fakeAngle[current_category].rightMin, 0, config->fakeAngle[current_category].rightLimit - 1, "%d");
    ImGui::PopItemWidth();
    ...
    if (config->rageAntiAim[current_category].roll) {
        ...
        ImGui::Checkbox("Using exploit pitch", &config->rageAntiAim[current_category].usingExpPitch);
        if (config->rageAntiAim[current_category].usingExpPitch)
...
void GUI::renderChamsWindow() noexcept
{
    ...
    ImGui::Combo("Material", &chams.material, "Normal\0Flat\0Animated\0Animated 2\0Platinum\0Glass\0Chrome\0Crystal\0Silver\0Gold\0Plastic\0Glow\0Pearlescent\0Metallic\0Snowflakes\0");
...
void GUI::renderStreamProofESPWindow() noexcept
{
    ...
    if (ImGui::BeginChild("##child", { 400.0f, 0.0f })) {
        ...
        if (currentCategory < 2) {
            ...
            ImGuiCustom::colorPicker("Weapon Icon", playerConfig.weaponIcon);
...
void GUI::renderVisualsWindow() noexcept
{
    ...
    ImGuiCustom::colorPicker("Advanced Scope", config->misc.advancedScope);
    ImGui::SameLine();
    ImGui::PushID("Better Scope");
    if (ImGui::Button("..."))
        ImGui::OpenPopup("");

    if (ImGui::BeginPopup("")) {
        ImGui::PushItemWidth(350.0f);
        ImGui::SliderFloat(" ", &config->misc.scopeOffset, 0.0f, 500.0f, "Lines Offset : %.2f");
        ImGui::SliderFloat("  ", &config->misc.scopeLength, 0.0f, 1000.0f, "Lines Length : %.2f");
        ImGui::EndPopup();
    }
    ImGui::PopID();
...
void GUI::renderMiscWindow() noexcept
{
    ...
    //ImGui::SameLine();
    ImGuiCustom::colorPicker("Indicators", config->misc.indicators, &config->misc.indicators.enabled);
    ImGuiCustom::colorPicker("Anti-Aim arrows", config->misc.aaArrows, &config->misc.aaArrows.enabled);
    ImGui::PopID();
    ...
    ImGui::Combo("FakeTags", &config->misc.faketag, "None\0Gamesense\0Skeet.cc\0Neverlose\0Nexus\0Rawetrip\0");
...
void rage_tab()
{
    ImGui::SetCursorPos(ImVec2(110, 80));
    ImGui::MenuChild("Primary", ImVec2(275, 275), NULL, NULL);
    {
    }
    ImGui::EndChild();
}
...
void render_tab()
{
    //ImGui::PushFont(IconFont);
    {
        ...
        if (ImGui::tab("Y", !tab)) tab = 0;

        ImGui::SetCursorPos(ImVec2{ 25, 170 });
        if (ImGui::tab("W", tab == 1)) tab = 1;

        ImGui::SetCursorPos(ImVec2{ 25, 230 });
        if (ImGui::tab("R", tab == 2)) tab = 2;

        ImGui::SetCursorPos(ImVec2{ 25, 290 });
        if (ImGui::tab("T", tab == 3)) tab = 3;

        ImGui::SetCursorPos(ImVec2{ 25, 350 });
        if (ImGui::tab("Q", tab == 4)) tab = 4;

        ImGui::SetCursorPos(ImVec2{ 25, 440 });
        if (ImGui::tab("E", tab == 5)) tab = 5;
    }
    //ImGui::PopFont();
...
void GUI::newMenu() noexcept
{
...

    {
        ...
        if (ImGui::Begin(("###"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar))
        {
            ...
            // Right
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(p.x + 74, p.y), ImVec2(p.x + 656, p.y + 560), ImColor(21, 21, 21), ImDrawFlags_RoundCornersBottomRight, 10);
            ImGui::GetWindowDrawList()->AddRect(ImVec2(p.x + 74, p.y), ImVec2(p.x + 656, p.y + 560), ImColor(35, 35, 35), ImDrawFlags_RoundCornersBottomRight, 10, 2.2);

            //logo
            ImGui::SetCursorPos(ImVec2(28, 28));
            //ImGui::Image(logo, ImVec2(35.f, 35.f));
            ...
            //user logo // steam foto
            ImGui::SetCursorPos(ImVec2(23, 500));
            //ImGui::Image(foto, ImVec2(45, 45));
        }
        ImGui::End();
    }
}
        ...
        ImGui::BeginChild("##Back", ImVec2{ 705, 445 }, false);
        {
            ...
            {   
                ImGui::BeginChild("##UP", ImVec2{ 700, 60 }, false);
                {
                    ...
                    //ImGui::SetCursorPos(ImVec2{ 255 - config->misc.textoffset, 2 + config->misc.textoffset});ImGui::TextColored(ImColor(70, 50, 240, 245), "Nexus \u043e\u0441\u0438\u043b\u0438\u0442\u044c\'\u0438");
                    //ImGui::SetCursorPos(ImVec2{ 255 + config->misc.textoffset, 2 - config->misc.textoffset}); ImGui::TextColored(ImColor(235, 5, 90, 245), "Nexus \u043e\u0441\u0438\u043b\u0438\u0442\u044c\'\u0438");
                    //ImGui::SetCursorPos(ImVec2{ 255, 2 }); ImGui::TextColored(ImColor(245, 245, 245, 245), "Nexus \u043e\u0441\u0438\u043b\u0438\u0442\u044c\'\u0438");
