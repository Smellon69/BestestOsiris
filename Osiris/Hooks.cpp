...
#include "../Osiris/ParticleCollection.h"
...
static HRESULT __stdcall present(IDirect3DDevice9* device, const RECT* src, const RECT* dest, HWND windowOverride, const RGNDATA* dirtyRegion) noexcept
{
    ...
    if (const auto& displaySize = ImGui::GetIO().DisplaySize; displaySize.x > 0.0f && displaySize.y > 0.0f) {
        ...
        Misc::StatusBar();
        Misc::advancedScope(ImGui::GetBackgroundDrawList());
        Misc::drawBombTimer();
        Misc::hurtIndicator();
        Misc::spectatorList();
        Misc::showKeybinds();
...
static void __stdcall setDrawColor(int r, int g, int b, int a) noexcept
{
    ...
    if (localPlayer && config->misc.advancedScope.enabled && (std::uintptr_t(_ReturnAddress()) == memory->staticScopeLensClear || std::uintptr_t(_ReturnAddress()) == memory->staticScopeLensBlurry))
        a = 0;
