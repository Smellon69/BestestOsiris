...
//ImGui::PushFont(GUI::fonts.tahoma16);
...
void Logger::getEvent(GameEvent* event) noexcept
{
    ...
    case fnv::hash("player_hurt"): {
        ...
        if (hurt != localPlayer->index() && attack == localPlayer->index())
        {
            ...
            log.text = std::string("[Nexus] Hit ") + player->getPlayerName() + " " + hitgroup + " for " + damage;
            //memory->clientMode->getHudChat()->printf(0, "[PassatHook]", player->getPlayerName(), " ", hitgroup, " for ", damage);
        }
        else if (hurt == localPlayer->index() && attack != localPlayer->index())
        {
            ...
            log.text = std::string("[Nexus] damage taken from ") + player->getPlayerName() + " for " + damage + " in " + hitgroup;
        }
        break;
    }
    case fnv::hash("bomb_planted"): {
        ...
        log.text = std::string("[Nexus] Bomb planted on ") + site + " by " + player->getPlayerName() + ", detonation in " + std::to_string(c4Timer->getFloat()) + " seconds";
        break;
    }
    case fnv::hash("hostage_follows"): {
        ...
        log.text = std::string("[Nexus] Hostage taken by ") + player->getPlayerName();
        break;
    }
    default:
        return;
...
void Logger::render(ImDrawList* drawList) noexcept
{
    ...
    for (int i = renderLogs.size() - 1; i >= 0; i--)
    {
        ...
        drawList->AddText(ImVec2{ 10.0f, 5.0f + static_cast<float>(20 * i) + 1.0f }, color & IM_COL32_A_MASK, renderLogs[i].text.c_str());
        drawList->AddText(ImVec2{ 10.0f, 5.0f + static_cast<float>(20 * i) + 1.0f }, color, renderLogs[i].text.c_str());
    }
