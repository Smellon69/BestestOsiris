...
static constexpr auto dispatchMaterial(int id) noexcept
{
    ...
    case 11: return glow;
    case 12: return pearlescent;
    case 13: return metallic;
    case 14: return snowflakes;
    }
}
...
static void initializeMaterials() noexcept
{
    ...
    {
        const auto kv = KeyValues::fromString("VertexLitGeneric", "$basetexture sprites/light_glow04 $nodecal 1 $model 1 $additive 1 $nocull 1 $wireframe 0 proxies { texturescroll { texturescrollvar $basetexturetransform texturescrollrate 0.75 texturescrollangle 75 } }");
        animated2 = interfaces->materialSystem->createMaterial("animated2", kv);
    ...
    {
        const auto kv = KeyValues::fromString("UnlitGeneric", "$basetexture dev/snowfield $additive 1 $envmap editor/cube_vertigo $envmapfresnel 1 $alpha 1 proxies { texturescroll { texturescrollvar $basetexturetransform texturescrollrate 0.10 texturescrollangle 270 } }");
        kv->setString("$envmaptint", "[0 0 0]");
        kv->setString("$envmapfresnelminmaxexp", "[.00000 .0 .0]");
        snowflakes = interfaces->materialSystem->createMaterial("snowflakes", kv);
    }
